...
export default function StudioUpdates() {
  ...
  const sorted = [...studio.updates].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())

  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-xl font-semibold text-ink-primary">Updates</h1>
        <p className="text-sm text-ink-secondary">Internal notes for the studio — not shared with clients</p>
      </div>
